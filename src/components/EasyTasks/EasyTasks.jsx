...
export default function EasyTasks() {
  ...
  return (
    <div className="mx-14 mt-5">
      <div
        className="relative bg-lightElements dark:bg-darkElements px-4 py-3 rounded-full flex flex-row justify-between items-center cursor-pointer z-20"
        onClick={() => {
      ...
      <div
        className={`bg-lightList dark:bg-darkList px-6 py-3 rounded-t-2xl rounded-b-lg shadow-lg z-0 overflow-hidden transition-all duration-300 ease-in-out ${
          showListTasks
            ? "-mt-9 pt-12 max-h-96 opacity-100"
            : "mt-0 pt-0 max-h-0 opacity-0 py-0"
        }`}
      >
        <ul>
          {tasks.map((task) => (
