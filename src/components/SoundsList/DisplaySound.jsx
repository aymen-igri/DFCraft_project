  ...
  return (
    <div className="fixed bottom-0 w-full bg-lightElements dark:bg-darkElements p-5 z-100 rounded-tl-2xl rounded-tr-2xl BtoT">
      <div className="flex flex-row items-center">
        ...
        />
        <div>
          <div className="text-light dark:text-dark font-medium">
            {sound.title}
          </div>
      ...
      <input
        ...
        style={{
          ...
          var(--selected-bar-color) ${(currentTime / duration) * 100}%,
          var(--bar-track-color) ${bufferProgress}%,
          var(--bar-color) ${(currentTime / duration) * 100}%)`,
        }}
      />
      ...
      <div className="flex justify-center">
        ...
        <button onClick={togglePlay} className="mr-3 ml-3" disabled={buffering}>
          {soundplayicon()}
        </button>
        <button disabled={buffering}>
