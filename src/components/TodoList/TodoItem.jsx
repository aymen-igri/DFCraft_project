...
export default function TodoItem({
  ...
  return (
    <div
      className={`bg-lightList dark:bg-darkList rounded-xl p-4 shadow-md hover:shadow-lg transition-all duration-200 ${todo.completed ? "opacity-60" : ""}`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3
              className={`font-semibold text-lightElements dark:text-darkElements ${todo.completed ? "line-through" : ""}`}
            >
              {todo.title.slice(0, 29) + (todo.title.length > 29 ? "..." : "")}
            </h3>
          ...
          <div className="flex flex-wrap gap-2">
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium text-white ${typeColors[todo.type]}`}
            >
              {taskType?.label}
            </span>
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium text-white ${priorityColors[todo.priority]}`}
            >
