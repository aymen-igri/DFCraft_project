...
const initialLang = supportedLanguages.includes(browserLang)
  ? browserLang
  : "en";
...
const TOKEN_CLASS_MAP = {
  ...
  darkProgressBar: [
    { cls: ".dark .bg-darkProgressBar", prop: "background-color" },
  ],
  darkSelectedBar: [
    { cls: ".dark .bg-darkSelectedBar", prop: "background-color" },
  ],
  darkPlaceHolder: [
  ...
  Object.entries(TOKEN_CLASS_MAP).forEach(([token, mappings]) => {
    ...
    mappings.forEach(({ cls, prop }) => {
      if (cls.startsWith(".dark ")) {
        lines.push(
          `${attr}.dark ${cls.slice(6)} { ${prop}: ${value} !important; }`,
        );
      } else {
...
export function SettingsProvider({ children }) {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.fontSize =
      sizes[settings.fontSize] ?? sizes.medium;
  }, [settings.fontSize]);
