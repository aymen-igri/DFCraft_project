...
const useBlockUrl = (BlockedItem, isRunning) => {
  ...
  console.log(
    "useBlockUrl called with BlockedItem:",
    BlockedItem,
    "isRunning:",
    isRunning,
  );
...
async function blockAccess(element) {
  ...
  if (element.urlBlocked) {
    ...
    const tab = tabs[0];
    console.log(
      tab && tab.url && tab.url.includes(element.url),
      "Checking if tab URL includes blocked URL",
    );
...
async function blockSound(element) {
  ...
  for (const tab of tabs) {
    ...
    console.log(
      tab.url && tab.url.includes(element.url),
      "Checking if tab URL includes sound blocked URL",
    );
    if (tab.url && tab.url.includes(element.url)) {
